...
void create_student()
{
	...
	oFile.open("student.dat", ios::binary | ios::app);  // Binary handles newline character. App for writing data in same file app: append
	stud.getdata();
...
void display_all()
{
	student stud;
	ifstream inFile;
	inFile.open("student.dat", ios::binary); // Binary handles newline character.
	if (!inFile)
	{
...
void change_student(int n)
{
	...
	fl.open("student.dat", ios::binary | ios::in | ios::out);  // ios in for input, ios out for output
	if (!fl)
	{
		cout << "File could not be opened. Press any Key to exit...";
		cin.ignore();
